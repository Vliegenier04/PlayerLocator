package vliegenier04.playerLocator.main.client.renderer.chunk;
...
public class ChunkProviderPL extends ChunkProviderClient implements IChunkProvider {
    private final playerLocatorWorld world;
    private final Chunk emptyChunk;
    private final Map<Long, ChunkPL> chunks = new ConcurrentHashMap<Long, ChunkPL>();

    public ChunkProviderPL(final playerLocatorWorld world) {
        super(world);
        this.world = world;
    ...
    public Chunk getLoadedChunk(final int x, final int z) {
        ...
        ChunkPL chunk = this.chunks.get(key);
        if (chunk == null) {
            chunk = new ChunkPL(this.world, x, z);
